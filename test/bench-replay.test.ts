...
const scanResult = { findings: [], suppressed: [], reminders: [], engines: [] };
...
describe("frozen full replay", () => {
  ...
  it("invalidates missing review context while the identical diff-only trial remains runnable", async () => {
    ...
    const diff = await runTrial(plan(fixture.base, fixture.head), [target], { repo: fixture.repo, traceRoot: join(fixture.root, "traces"), runner, scanFn: (async () => scanResult) as never });
