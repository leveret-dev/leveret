...
#   --base REF  worktree base (default origin/HEAD, else HEAD)
...
main() {
	...
	scrub_git_env "$0"
	kind='' nn='' title='' do_worktree=0 do_claim=0 path='' path_set=0 absolute_path=0 base=''
	...
	if [ -z "$base" ]; then
		base=$(git -C "$root" symbolic-ref --quiet refs/remotes/origin/HEAD 2>/dev/null) || base=''
